...
func AnalyticsOperatorRepo(ctx context.Context, wg *sync.WaitGroup, mongoIp string) (hostPort string, ipAddress string, err error) {
	...
	container, err := pool.RunWithOptions(&dockertest.RunOptions{
		Repository: "fgseitsrancher.wifa.intern.uni-leipzig.de:5000/analytics-operator-repo",
		Tag:        "dev",
		Env: []string{
			"MONGO_ADDR=" + mongoIp,
		},
	...
	go Dockerlog(pool, ctx, container, "ANALYTICS-OPERATOR-REPO")
	err = pool.Retry(func() error {
		log.Println("try AnalyticsOperatorRepo connection...")
