...
func AnalyticsParserMock(ctx context.Context, wg *sync.WaitGroup) (url string) {
	server := &httptest.Server{
		Config: &http.Server{Handler: http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
			log.Println("DEBUG: AnalyticsParserMock: got request", request.Method, request.URL.String())
			if strings.Contains(request.URL.Path, "flow/") {
				parts := strings.Split(request.URL.Path, "/")
				id := parts[len(parts)-1]
				json.NewEncoder(writer).Encode(Pipeline{FlowId: id})
			}
		})},
	}
